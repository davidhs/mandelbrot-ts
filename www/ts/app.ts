class App {
    ...
    public getRegion(code: string): {x: number, y: number, w: number, h: number} {
    ...
    public refresh(): void {
        ...
        for (let y = 0; y < h; y += 1) {
            for (let x = 0; x < w; x += 1) {
                ...
                //
                this.imageData.data[idx] = this.imageDataBuffer[ib];
                this.imageData.data[idx + 1] = this.imageDataBuffer[ib + 1];
                this.imageData.data[idx + 2] = this.imageDataBuffer[ib + 2];
                this.imageData.data[idx + 3] = this.imageDataBuffer[ib + 3];
    ...
    public requestJob(workerIndex: number): void {
        ...
        if (!running && !node.flag) {
            ...
            if (this.VERBOSE) { console.log("No jobs for you"); }
            this.updated = false;
            return null;
        }
    }
    ...
    public workerCallback(e: MessageEvent): void {
        ...
        if (this.VERBOSE) { console.log("Displaying [" + message.part + "]..."); }
    ...
    public static startJob<T>(worker: Worker, message: Object, transferList: Array<T>, callback: (e: MessageEvent) => void): void {
